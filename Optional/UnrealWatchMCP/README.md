...
Tiny **host-side** MCP server that detects Unreal Editor process state, freezes, dialogs, Crash Report Client, and Restore Packages from outside the game thread.
...
| `dismiss_unreal_blocker` | Safe dismiss: Escape/Cancel/Don't Restore/close CRC. Never Delete without `allow_destructive=true`. |
...
**Important:** Unreal “Message dialog” / compile-error popups are usually owned `UnrealWindow` Slate windows — **not** classic `#32770` Win32 dialogs. Crash reporter is a **separate process**.
...
3. If `modal_blocked` / `crash_reporter` / `restore_packages` → `dismiss_unreal_blocker` (safe defaults).
...
Destructive prompts (`Don't Save`, `Delete`, checkout, overwrite) and Restore Packages / Crash Reporter are **never** auto-clicked in allowlist mode.
